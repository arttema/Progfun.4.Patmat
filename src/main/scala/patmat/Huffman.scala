...
object Huffman {
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] =
    freqs.sortBy(_._2).map((x: (Char, Int)) => Leaf(x._1, x._2))
   ...
   */
  def until(map: (List[CodeTree]) => Boolean, reduce: (List[CodeTree]) => List[CodeTree])
           (in: List[CodeTree]): CodeTree = {

    def fore(acc: List[CodeTree]): List[CodeTree] = {
      if (map(acc)) acc
      else fore(reduce(acc))
    }
    fore(in)(0)
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    ...
    def fore(acc: List[Char], bits: List[Bit]): List[Char] = {
      if (bits.isEmpty)
        acc
      else {
        val charDepth: (Char, Int) = decodeChar(tree, bits, 0)
        fore(acc ++ List(charDepth._1), bits drop charDepth._2)
      }
    }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] =
    table.find( _._1 == char).getOrElse(('d',List()))._2
